...
const canvas2Cards: ContentCard[] = [
  {
    ...
    text:
      '生成式 AI 正以前所未有的速度重塑全球科技與半導體產業格局。NVIDIA 的 H200 與 GB200 系列 GPU 供不應求，交期延至 2026 Q3；TSMC CoWoS 封裝產能成為整個 AI 供應鏈的瓶頸所在。微軟、Google、Meta 與 Amazon 合計在 2025 年投入超過 $2,000 億美元的 AI 資本支出。與此同時，模型效率化趨勢（如 MoE 架構）正在改變推論端的硬體需求，對 ASIC 設計公司（如 Trainium、TPU）形成挑戰。AI PC 滲透率在 2026 年預計突破 35%，拉動 DRAM 需求大幅升溫。',
...
const canvas3Cards: ContentCard[] = [
  {
    ...
    text:
      '2026 年 1 月初，美國與伊朗之間的外交緊張再度升溫，霍爾木茲海峽的通行安全引發全球供應鏈高度警戒。該海峽每日通過約 2,100 萬桶原油，佔全球海運原油供應的近 21%。伊朗已發出警告，若衝突升級，不排除封鎖海峽。對半導體產業而言，特殊製程氣體（氖氣、氪氣）及稀土元素的替代供應鏈尚不成熟，短期內料將面臨嚴峻壓力。TSMC已啟動緊急庫存盤點，並加速與日本、歐洲供應商的備援協議洽商。',
  ...
  },
  {
    ...
    tableData: {
      ...
      rows: [
        ...
        ['半導體特殊氣體', '🟠 中高', 'TSMC、聯電、力積電', '日本、歐洲供應商', '2–4 個月'],
...
const canvas1Tasks: Task[] = [
  ...
  {
    id: 't1-2',
    title: '確認TSMC在 iPhone 供應鏈中佔比與未來展望',
    status: 'todo',
    assignee: m2,
    due: '2026-02-15',
    priority: 'medium',
...
const canvas3Tasks: Task[] = [
  {
    id: 't3-1',
    title: '評估TSMC特殊氣體庫存水位（目前存量可支撐幾週）',
    status: 'in-progress',
    assignee: m5,
    due: '2026-01-25',
    priority: 'high',
...
export const canvases: Canvas[] = [
  ...
  },
  {
    ...
    tags: ['AI', 'NVIDIA', '半導體', '雲端', 'TSMC', '產業趨勢'],
