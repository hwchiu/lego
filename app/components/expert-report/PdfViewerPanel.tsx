...
const LibraryPdfViewerClient = dynamic(
  () => import('@/app/components/expert-report/LibraryPdfViewerClient'),
  { ssr: false },
);
...
export default function PdfViewerPanel({ report, viewMode }: PdfViewerPanelProps) {
  ...
  const labels = {
    ...
    library: { zh: 'Reader', en: 'Reader' },
  };

  if (!report) {
    return (
      <div className={`er-pdf-panel${viewMode === 'full' ? ' er-pdf-panel--library' : ''}`}>
    ...
    <div className={`er-pdf-panel${viewMode === 'full' ? ' er-pdf-panel--library' : ''}`}>
      <div className="er-pdf-header">
        ...
        <span className="er-pdf-label">
          {viewMode === 'full' ? labels.library[lang] : labels.preview[lang]}
        </span>
      </div>
      {viewMode === 'full' ? (
