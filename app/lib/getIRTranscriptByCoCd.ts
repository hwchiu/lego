...
/**
 * Simulated API: getIRTranscriptByCoCd
 *
 * Fetches IR Transcript HTML entries from the local data store,
 ...
 *   GET /api/ir-transcripts?co_cd={coCd}
 *
 * @param coCd  Company code / ticker symbol, e.g. "AAPL"
 * @returns Promise resolving to an array of IrTranscriptHtmlEntry
 */
export async function getIRTranscriptByCoCd(
  coCd: string,
): Promise<IrTranscriptHtmlEntry[]> {
  // Simulate network latency (remove when switching to real API)
  // await new Promise((r) => setTimeout(r, 200));

  const filtered = IR_TRANSCRIPT_HTML_ENTRIES.filter(
