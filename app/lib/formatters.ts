 ...
 *
 * - `valueM >= 1000` (≥ 1 billion)  → `"$2.5B"` (uses `decimals` for the B tier, default 1)
 * - `1 <= valueM < 1000`             → `"$450.00M"` (2 decimal places + thousands separator)
 * - `valueM < 1`                     → `"$0.50M"` (2 decimal places)
 *
 * @param valueM   Numeric value in millions USD
 * @param decimals Decimal places for the B-tier label (default: 1)
 * @returns Formatted string, e.g. "$2.5B", "$450.00M", "$0.50M"
 *
 * @example
 * formatUsdM(2500)       // "$2.5B"
 * formatUsdM(2500, 2)    // "$2.50B"
 * formatUsdM(450)        // "$450.00M"
 * formatUsdM(1234.5)     // "$1,234.50M"
 * formatUsdM(0.5)        // "$0.50M"
 */
export function formatUsdM(valueM: number, decimals = 1): string {
  if (valueM >= 1000) return `$${(valueM / 1000).toFixed(decimals)}B`;
  if (valueM >= 1) return `$${valueM.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}M`;
 ...
 *
 * - `valueM >= 1000` (≥ 1 billion)  → `"$2.5B"` (uses `decimals` for the B tier, default 1)
 * - `1 <= valueM < 1000`             → `"$450M"` (integer with thousands separator)
 * - `valueM < 1`                     → `"$0.50M"` (2 decimal places)
 *
 * @param valueM   Numeric value in millions USD
 * @param decimals Decimal places for the B-tier label (default: 1)
 * @returns Formatted string, e.g. "$2.5B", "$450M", "$0.50M"
 *
 * @example
 * formatUsdM(2500)    // "$2.5B"
 * formatUsdM(2500, 2) // "$2.50B"
 ...
 *
 * @param value          Numeric value
 * @param fractionDigits Minimum / maximum fraction digits (default: 0)
 * @returns `"1,234,567"` or `"1,234.50"` (when fractionDigits = 2)
 *
 * @example
